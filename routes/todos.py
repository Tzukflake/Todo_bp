...
def create_todo():
    ...
    todo = todo_service.create(title=title, description=description, due_date=due_date, user_id=user_id, category_ids=category_id)
...
def update_todo(todo_id):
    ...
    if due_date:
        due_date = datetime.strptime(due_date, '%Y-%m-%d %H:%M:%S')
    todo = todo_service.update(todo_id=todo_id, title=title, description=description, is_completed=is_completed, due_date=due_date,  category_ids=category_id)
    todo_dict = todo.to_dict()
